 ...
 * translation type.
 */
static void intel_pte_config_first_level(struct intel_iommu *iommu,
					 struct pasid_entry *pte,
				  ...
				  u32 pasid, u16 did, int flags)
{
	...
	intel_pte_config_first_level(iommu, pte, pgd, did, flags);
...
int intel_pasid_convert_to_first_level(struct intel_iommu *iommu,
				       struct device *dev, pgd_t *pgd,
				       u32 pasid, u16 did, int flags)
{
	struct pasid_entry *pte;
	...
	intel_pasid_flush_present(iommu, dev, pasid, old_did, pgtt,
				  pte, INTEL_PASID_TEARDOWN_DRAIN_PRQ);
 ...
 */
static void intel_pte_config_second_level(struct intel_iommu *iommu,
					  struct pasid_entry *pte,
				   ...
				   struct device *dev, u32 pasid)
{
	...
	intel_pte_config_second_level(iommu, pte, pgd_val, agaw,
...
int intel_pasid_convert_to_second_level(struct intel_iommu *iommu,
					struct dmar_domain *domain,
					struct device *dev, u32 pasid)
{
	struct pasid_entry *pte;
	u16 did, old_did, pgtt;
	struct dma_pte *pgd;
	u64 pgd_val;
	int agaw;
 ...
 */
static void intel_pte_config_pass_through(struct intel_iommu *iommu,
				   ...
				   struct device *dev, u32 pasid)
{
	...
	intel_pte_config_pass_through(iommu, pte, did);
	spin_unlock(&iommu->lock);
...
int intel_pasid_convert_to_pass_through(struct intel_iommu *iommu,
					struct device *dev, u32 pasid)
{
	u16 did = FLPT_DEFAULT_DID, old_did, pgtt;
	struct pasid_entry *pte;
...
static void intel_pte_config_nestd(struct intel_iommu *iommu,
				   struct pasid_entry *pte,
			     ...
			     u32 pasid, struct dmar_domain *domain)
{
	...
	intel_pte_config_nestd(iommu, pte, s1_cfg, s2_domain, did);
	spin_unlock(&iommu->lock);
...
int intel_pasid_convert_to_nested(struct intel_iommu *iommu,
				  struct device *dev,
				  u32 pasid, struct dmar_domain *domain)
{
	u16 did = domain_id_iommu(domain, iommu), old_did, pgtt;
	struct iommu_hwpt_vtd_s1 *s1_cfg = &domain->s1_cfg;
	struct dmar_domain *s2_domain = domain->s2_domain;
	struct pasid_entry *pte;
