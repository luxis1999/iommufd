				    ...
				    u32 pasid, struct iommu_domain *old)
{
	...
	if (old)
		return intel_pasid_convert_to_first_level(iommu, dev,
							  (pgd_t *)pgd, pasid,
...
{
	if (old)
		return intel_pasid_convert_to_second_level(iommu, domain,
							   dev, pasid);
	else
...
{
	if (old)
		return intel_pasid_convert_to_nested(iommu, dev,
						     pasid, domain);
	else
			      ...
			      struct iommu_domain *old)
{
	...
	/* Nested type domain should check its parent domain */
	...
	if (domain_type_is_nested(dmar_domain))
		ret = domain_setup_nested(iommu, dmar_domain,
					  dev, pasid, old);
